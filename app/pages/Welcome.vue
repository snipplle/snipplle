          ...
          <UCard class="relative w-full min-w-md max-w-md rounded-lg">
            <div class="space-y-4">
              <div>
                ...
                <p class="text-sm text-gray-500">Create a new workspace to start organizing your snippets.</p>
              </div>
...
<script setup lang="ts">
  ...
  const schema = z.object({
    name: z.string('Workspace name is required').min(1, 'Workspace name must be at least 1 character long')
  })

  type Schema = z.output<typeof schema>
  
  async function createWorkspace(payload: FormSubmitEvent<Schema>) {
    try {
